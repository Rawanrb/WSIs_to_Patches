...
SUMMARY_TITLE_FONT_PATH = 'FreeSans.ttf'
...
def score_tiles(slide_num, np_img=None, dimensions=None, small_tile_in_tile=False):
  ...
  tile_sum = TileSummary(slide_num=slide_num,
                         ...
                         tissue_percentage=filters.tissue_percent(np_img),
  ...
  #this is to set the path to the mask of the slide
  #mask_path = slide.get_mask_image_result(slide_num)
  #np_img_mask = slide.open_image_np(mask_path)

  if save_tiles_data == True:
    import openpyxl
    wb = openpyxl.load_workbook(slide.xlsx_name)
    sheet = wb.get_sheet_by_name(slide.sheet_name)
    sheet['A' + str(count + 1)].value = 'count'
    sheet['B' + str(count + 1)].value = 'score'
    sheet['C' + str(count + 1)].value = 'o_r_s'
    sheet['D' + str(count + 1)].value = 'o_c_s'
    sheet['E' + str(count + 1)].value = 'o_r_e'
    sheet['F' + str(count + 1)].value = 'o_c_e'
    sheet['G' + str(count + 1)].value = 'color_factor'
    sheet['H' + str(count + 1)].value = 's_and_v_factor'
    sheet['I' + str(count + 1)].value = 'quantity_factor'
    sheet['J' + str(count + 1)].value = 'tissue_percent'
    #sheet['K' + str(count + 1)].value = 'tissue_quantity'
    #sheet['L' + str(count + 1)].value = 'stain_vector'
  ...
  for t in tile_indices:
    ...
    t_p = filters.tissue_percent(np_tile)
    ...
    if save_tiles_data == True:
      #change to score depend on the annotation number on the mask
      count = 0
      #np_tile_mask = np_img_mask[r_s:r_e, c_s:c_e]
      #score = unique_count_app(np_tile_mask)
      if(score > 0 ):
      #if(score > 0 and amount == TissueQuantity.HIGH): #if the score is tumour or stroma
          #take these dimentions and save them  to excel sheet for later extraction
          #o_c_s, o_r_s,o_c_e, o_r_e
        count += 1  # add one if the score is not background (0)
        sheet['A' + str(count + 1)].value = count
        sheet['B' + str(count + 1)].value = score
        sheet['C' + str(count + 1)].value = o_r_s
        sheet['D' + str(count + 1)].value = o_c_s
    ...
    tile = Tile(tile_sum, slide_num, np_scaled_tile, count, r, c, r_s, r_e, c_s, c_e, o_r_s, o_r_e, o_c_s,
                o_c_e, t_p, color_factor, s_and_v_factor, quantity_factor, score, prob=0)
    tile_sum.tiles.append(tile)

  if save_tiles_data == True:
     wb.save(slide.xlsx_name)
...
def unique_count_app(np_img):
    colors, count = np.unique(np_img, return_counts=True)
    return colors[count.argmax()]

#def amount_of_stain(np_img):
...
def generate_tiled_html_result(slide_nums, tile_summaries_dict, data_link):
  ...
  if not slide.TILE_SUMMARY_PAGINATE:
    html = ""
    html += filters.html_header("Tiles")
    ...
    for page_num in range(1, num_pages + 1):
      ...
      html = ""
      html += filters.html_header("Tiles, Page %d" % page_num)
...
class Tile:
  ...
  def __init__(self, tile_summary, slide_num, np_scaled_tile, tile_num, r, c, r_s, r_e, c_s, c_e, o_r_s, o_r_e, o_c_s,
               o_c_e, t_p, color_factor, s_and_v_factor, quantity_factor, score, prob):
