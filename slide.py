...
sheet_name = 'Sheet1'
BASE_DIR = os.path.join(os.sep, "media","bialab","7c33ac31-d9d1-4f8d-aed4-ef4049a63dc2", "tialab_rawan","Datasets","TNBC_Dataset_")
TRAIN_PREFIX = "TNBC_AUS_"
SRC_TRAIN_DIR = os.path.join(BASE_DIR, "Pilot_study")
#SRC_TRAIN_DIR = os.path.join(BASE_DIR, "Annotations","tif")
SRC_TRAIN_EXT = "ndpi"
#SRC_TRAIN_EXT = "tif"
...
#DEST_TRAIN_DIR = os.path.join(BASE_DIR, "training_mask_" + DEST_TRAIN_EXT)
THUMBNAIL_SIZE = 300
THUMBNAIL_EXT = "jpg"

MASK_DIR = os.path.join(BASE_DIR,"Annotations", "pngs")
...
PATCH_EXT = "png" 
PATCH_DIR = os.path.join(BASE_DIR, "patches___" + PATCH_EXT)
...
STATS_DIR = os.path.join(BASE_DIR, "ndpi_stats")
...
def get_training_thumbnail_path(slide_number, large_w=None, large_h=None, small_w=None, small_h=None):
    ...
    if large_w is None and large_h is None and small_w is None and small_h is None:
        wilcard_path = os.path.join(
            DEST_TRAIN_THUMBNAIL_DIR, TRAIN_PREFIX + padded_sl_num + "*." + THUMBNAIL_EXT)
        img_path = glob.glob(wilcard_path)[0]
    else:
...
def get_filter_image_path(slide_number, filter_number, filter_name_info):
    ...
    if not os.path.exists(dir):
        os.makedirs(dir)
    img_path = os.path.join(dir, get_filter_image_filename(
        slide_number, filter_number, filter_name_info))
    return img_path

def get_patch_path(tile,count,scale_factor,w,h):

    dir = PATCH_DIR
    if not os.path.exists(dir):
        os.makedirs(dir)
    patch_path = os.path.join(dir, get_patch_image_filename(
        tile,count,scale_factor,w,h))
    return patch_path

def get_patch_image_filename(tile,count,scale_factor,w,h):
  ext = PATCH_EXT

  t = tile

  padded_sl_num = str(t.slide_num).zfill(3)
  padded_fi_num = str(t.score).zfill(3)

  patch_filename = os.path.join(padded_sl_num,padded_fi_num,str(w) + "x" + str(h),padded_sl_num + \
        "_" + str(count) + "." + ext)
...
def get_filter_image_filename(slide_number, filter_number, filter_name_info, thumbnail=False):
    ...
    img_filename = TRAIN_PREFIX + padded_sl_num + "-" + padded_fi_num + \
        "-" + FILTER_SUFFIX + filter_name_info + "." + ext
    return img_filename
...
def get_tile_summary_image_path(slide_number):
    ...
    img_path = os.path.join(
        TILE_SUMMARY_DIR, get_tile_summary_image_filename(slide_number))
    return img_path
...
def get_tile_summary_thumbnail_path(slide_number):
    ...
    img_path = os.path.join(TILE_SUMMARY_THUMBNAIL_DIR, get_tile_summary_image_filename(
        slide_number, thumbnail=True))
    return img_path
...
def get_tile_summary_on_original_image_path(slide_number):
    ...
    img_path = os.path.join(TILE_SUMMARY_ON_ORIGINAL_DIR,
                            get_tile_summary_image_filename(slide_number))
    return img_path
...
def get_top_tiles_on_original_image_path(slide_number):
    ...
    img_path = os.path.join(TOP_TILES_ON_ORIGINAL_DIR,
                            get_top_tiles_image_filename(slide_number))
    return img_path
...
def get_top_tiles_image_path(slide_number):
    ...
    img_path = os.path.join(
        TOP_TILES_DIR, get_top_tiles_image_filename(slide_number))
    return img_path
...
def get_top_tiles_thumbnail_path(slide_number):
    ...
    img_path = os.path.join(TOP_TILES_THUMBNAIL_DIR, get_top_tiles_image_filename(
        slide_number, thumbnail=True))
    return img_path
...
def get_tile_data_path(slide_number):
    ...
    file_path = os.path.join(
        TILE_DATA_DIR, get_tile_data_filename(slide_number))
    return file_path
...
def get_mask_image_result(slide_number):

    padded_sl_num = str(slide_number).zfill(3)
    training_img_path = get_training_image_path(slide_number)
    large_w, large_h, small_w, small_h = parse_dimensions_from_image_filename(
        training_img_path)
    img_path = os.path.join(MASK_DIR, TRAIN_PREFIX + padded_sl_num + "-" + str(
        SCALE_FACTOR) + "x-" + FILTER_SUFFIX + str(large_w) + "x" + str(large_h) + "-" + str(small_w) + "x" + str(
...
def slide_stats():
    ...
    stats_string = ""
    stats_string += "%-11s {:14,d} pixels (slide #%d)".format(
        max_width) % ("Max width:", which_max_width)
    stats_string += "\n%-11s {:14,d} pixels (slide #%d)".format(
        max_height) % ("Max height:", which_max_height)
    stats_string += "\n%-11s {:14,d} pixels (slide #%d)".format(
        max_size) % ("Max size:", which_max_size)
    stats_string += "\n%-11s {:14,d} pixels (slide #%d)".format(
        min_width) % ("Min width:", which_min_width)
    stats_string += "\n%-11s {:14,d} pixels (slide #%d)".format(
        min_height) % ("Min height:", which_min_height)
    stats_string += "\n%-11s {:14,d} pixels (slide #%d)".format(
        min_size) % ("Min size:", which_min_size)
...
def slide_info(display_all_properties=False):
    ...
    sheet['F' + str(slide_num + 1)].value = 'objective_power'
    ...
    for slide_num in range(1, num_train_images):
        ...
        sheet['F' + str(slide_num + 1)].value = str(objective_power)
